...
function initAuth() {
    ...
    let isHOUser = false;
    try {
        const savedSession = JSON.parse(localStorage.getItem("jccb_user_session") || sessionStorage.getItem("jccb_user_session") || "{}");
        if (savedSession && (savedSession.code === "99" || savedSession.isHO || savedSession.isAdmin)) {
            isHOUser = true;
        }
    } catch (e) {}
...
async function loadAllData(showSpinner = true) {
    ...
    if (window.PostgresSync && window.PostgresSync.isConnected()) {
        try {
            const [goldRes, actRes] = await Promise.all([
                window.PostgresSync.runNeonQuery("SELECT payload FROM jccb_gold_loans ORDER BY updated_at DESC;"),
                window.PostgresSync.runNeonQuery("SELECT * FROM jccb_branch_activity ORDER BY created_at DESC LIMIT 100;")
            ]);

            if (goldRes && goldRes.rows && goldRes.rows.length > 0) {
                mgmtState.loans = goldRes.rows.map(r => r.payload);
            }
            if (actRes && actRes.rows && actRes.rows.length > 0) {
                mgmtState.auditLogs = actRes.rows;
                renderAuditLogs();
            }
            updateTopMetadata();
            return;
        } catch (pgErr) {
            console.warn("[Management] Neon load error:", pgErr);
        }
    }
...
async function loadLocalFallbackData() {
    // Check Neon first
    if (window.PostgresSync) {
        try {
            const goldRes = await window.PostgresSync.runNeonQuery("SELECT payload FROM jccb_gold_loans ORDER BY updated_at DESC;");
            if (goldRes && goldRes.rows && goldRes.rows.length > 0) {
                mgmtState.loans = goldRes.rows.map(r => r.payload);
                updateTopMetadata();
                return;
    ...
    try {
        const saved = JSON.parse(localStorage.getItem("jccb_gold_system_state_v2") || localStorage.getItem("jccb_gold_state") || "{}");
