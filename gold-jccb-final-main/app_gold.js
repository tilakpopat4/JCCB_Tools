...
function getActiveSession() {
    try {
        const raw = sessionStorage.getItem("jccb_active_session") || sessionStorage.getItem("jccb_user_session") || localStorage.getItem("jccb_user_session");
        if (raw) {
            const parsed = JSON.parse(raw);
            if (parsed && (parsed.code || parsed.name)) {
                if (!parsed.code) parsed.code = "99";
                if (!parsed.name) parsed.name = "99 HEAD OFFICE";
                return parsed;
            }
        }
    } catch (e) { }
    ...
    try {
        sessionStorage.setItem("jccb_active_session", JSON.stringify(defaultSess));
        localStorage.setItem("jccb_user_session", JSON.stringify(defaultSess));
    } catch(e) {}
    return defaultSess;
}
...
function promptMandatoryPasswordChange(branchObj) {
    // Compulsory password change disabled by user request
    return;
}
...
function showApp() {
    ...
    const headerBranchSpan = document.getElementById("header-branch-name");
    ...
    if (headerBranchSpan && state.currentSession) headerBranchSpan.textContent = state.currentSession.name;
    if (headerRoleBadge && state.currentSession) {
        headerRoleBadge.textContent = (state.currentSession.code === "99" || state.currentSession.isAdmin) ? "👑 Super Admin" : "🏢 Branch User";
    }
...
function compressImageFile(file, maxDim = 320, quality = 0.55) {
...
function initImageCropper() {
    ...
    async function processImageFile(file, target) {
        ...
        const maxDim = target === "customer" ? 260 : 360;
        const optimizedBase64 = await compressImageFile(file, maxDim, 0.55);
            ...
            if (cropperInstance) {
                try {
                    const canvas = cropperInstance.getCroppedCanvas({
                        width: currentPhotoTarget === "customer" ? 260 : 360,
                        height: currentPhotoTarget === "customer" ? 260 : 270
                    });
                    if (canvas) {
                        const croppedBase64 = canvas.toDataURL("image/jpeg", 0.55);
