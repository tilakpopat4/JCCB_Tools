...
function initAuth() {
    try {
        const savedSession = localStorage.getItem('jccb_user_session') || sessionStorage.getItem('jccb_user_session');
        if (savedSession) {
...
function enterApp() {
    try {
        ...
        const roleBadgeElem = document.getElementById("current-user-role-badge");

        if (state.currentSession) {
            const branchText = `${state.currentSession.code} ${state.currentSession.name}`;
            if (userBranchElem) userBranchElem.textContent = branchText;
            if (welcomeBranchElem) welcomeBranchElem.textContent = state.currentSession.name;
            if (roleBadgeElem) roleBadgeElem.textContent = state.currentSession.isAdmin ? '👑 Super Admin' : '🏢 Branch User';
        }

        // Apply Central Role Permissions
        const isAdmin = (state.currentSession && (state.currentSession.isAdmin || state.currentSession.code.toString() === "99"));
